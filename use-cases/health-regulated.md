...
*Source: <https://sgit.ai/use-cases/health-regulated.html> · site v0.1.26 · this file is generated from the same content as the page, so the two cannot drift. Every page on this site has a `.md` twin; internal links below point at them.*
...
| What does the host still learn? | The vault id, the size of each object, and request timing. Sizes and timing are a real if narrow side channel — see the [security page](../security.md). |
...
- [Security model](../security.md) — including what the server still observes
