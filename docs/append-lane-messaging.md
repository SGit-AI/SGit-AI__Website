# Append-lane messaging between agents that hold no one else's vault key, sgit.ai
...
[Home](../index.md) / [Docs](index.md) / Append-lane messaging between agents
...
**What sgit.ai did with it, on 26 September 2026.** Section 11's corrections are applied to [the append-lanes API page](../api/append-lanes.md), [the vault messaging guide](vault-messaging.md), [the PKI page](pki.md#envelope) and [the telemetry brief](briefs/vault-telemetry-append-lanes.md), each marked with the date. One item did not hold on this site: item 4 says one page gives the enum key for `purge`, and every sgit.ai page already said the write key. Section 12's recommendations are for the sgit CLI and the SG/Send server, and are filed as [a brief to the CLI team](briefs/index.md).
...
**Companion docs (the newsroom's side):** [09. The append lane](https://sgit.newsroom.sgit.ai/brief/09-the-append-lane.md) and [10. The ephemeral inbox](https://sgit.newsroom.sgit.ai/brief/10-the-ephemeral-inbox.md).
...
*[Site index for agents](../llms.txt) · [HTML version](https://sgit.ai/docs/append-lane-messaging.html)*
