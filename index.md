...
*Source: <https://sgit.ai/index.html> · site v0.6.0 · this file is generated from the same content as the page, so the two cannot drift. Every page on this site has a `.md` twin; internal links below point at them.*
...
Four of **32 published vaults**. Each opens with a read key printed on its page, no account, nothing to install, and the server that stores it cannot read it. [See all 32 →](demos/vaults/index.md)
...
158site releases, each verified live before it was called done

32vaults published with a deliberately public read key
