---
title: graphs.sgit.ai
domain: graphs.sgit.ai
url: https://sgit-ai.github.io/SGit-AI__Website__Graphs/
tagline: A node is just a node — meaning lives in the edges
...
hero: graphs-home.webp
tags: graphs, semantics, method
---
...
!shot graphs-home.webp | images/ | The home page: the thesis, the sourced quote, and the disclaimer immediately under it.
...
A site about graphs whose first move is to disown the product category a reader expects to be sold is doing the same thing [sg-sentinel.sgit.ai](/network/sg-sentinel.html) does with its `NOT BUILT` banner. The house rule holds: publish the work, then say precisely what it is worth.
...
!shot graphs-grammar.webp | images/ | The grammar page: the one-screen version, the verb/inverse table, and the ban on `relates-to`.
...
!shot graphs-shipped.webp | images/ | The reality page: the ships-versus-argued table, with the absence of a graph database stated at the top.
...
**The query API is the vault's read model.** `sg.history.log/list/read` handed to untrusted sandboxed code is exactly the surface the [vault demos](/demos/index.html) run on. A graph query interface that an app you do not trust may call is a zero-knowledge story as much as a graph one.
...
> **One correction to pass upstream.** The graphs site's footer links to `sentinel.sgit.ai`, which does not resolve. The site is at [`sg-sentinel.sgit.ai`](/network/sg-sentinel.html) — the `sg-` prefix is load-bearing. Checked 2026-08-21.
