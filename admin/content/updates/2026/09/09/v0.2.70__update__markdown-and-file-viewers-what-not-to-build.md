...
Two of the most common things an agent is asked to add to a vault are a **markdown viewer** and a **file/folder browser with raw views**. Both already exist in the vault platform, and most requests for them are answered by publishing files in the right shape and writing no code at all. [**The brief now says so.**](/briefs/markdown-and-file-viewers.html)

It is a decision rather than a syntax reference — [the syntax reference already existed](/vault/content-authoring.html). A four-rung ladder, with the instruction to stop at the first rung that works:
...
3. **Build a site viewer** only when the content must live outside a vault host — [the decks brief](/briefs/vault-decks-on-a-site.html) covers that case.
