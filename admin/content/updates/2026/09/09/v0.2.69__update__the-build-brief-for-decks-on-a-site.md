...
Two releases documented this from the inside: [reading one file out of a vault](/docs/vault/reading-a-vault-file.html) explains the mechanism, and the deck pages demonstrate it. Neither told another agent how to do it to *their* vault. [**That brief now exists.**](/docs/briefs/vault-decks-on-a-site.html)

It follows the shape of [the telemetry brief](/docs/briefs/vault-telemetry-append-lanes.html) — the shape that worked, because another team's agent built a vault from it and then sent back a review that corrected it.
