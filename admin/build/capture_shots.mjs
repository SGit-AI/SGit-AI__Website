/* capture_shots.mjs — drive a published vault in a real browser and save cropped
   ...
   passed to this tool.
     ...
     node admin/build/capture_shots.mjs [--mirror] [--only <name>]
...
const OUT_DIR  = path.join(ROOT, 'assets/shots');
...
const SHOTS = [
  { name: 'gallery-row', cred: ALGARVE, surface: 'app', viewport: [1600, 1000],
    steps: [{ appScroll: '.grid-row' }, { wait: 1200 }],
    target: 'app', sel: '.grid-row' },

  { name: 'lightbox', cred: ALGARVE, surface: 'app', viewport: [1600, 1000],
  ...
  { name: 'hud-vault', cred: ALGARVE, surface: 'app', viewport: [1800, 1000],
  ...
  { name: 'hud-repl', cred: ALGARVE, surface: 'app', viewport: [1800, 1000],
  ...
  { name: 'source-view', cred: ALGARVE, surface: 'vault', viewport: [1700, 1000],
  ...
  { name: 'sgit-history', cred: ALGARVE, surface: 'vault', viewport: [1700, 1000],
...
async function appFrame(page, must = '.grid-row', timeout = 90000) {
  const t0 = Date.now();
  while (Date.now() - t0 < timeout) {
    for (const f of page.frames()) {
      if (f === page.mainFrame()) continue;
      const ok = await f.evaluate(m => !!document.querySelector(m), must).catch(() => false);
      if (ok) return f;
    }
...
async function capture(browser, shot) {
  ...
  if (shot.surface === 'vault') {
    ...
    if (!await appFrame(page)) throw new Error('app never rendered');
  }
  ...
  const out = path.join(OUT_DIR, shot.name + '.webp');
  let img = sharp(buf);
  if (shot.trim) {
  ...
  console.log(`  ✓ ${shot.name.padEnd(14)} ${kb} KB`);
  await ctx.close();
}
