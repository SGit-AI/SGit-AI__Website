...
*Source: <https://sgit.ai/demos/fractal-graphs/index.html> · site v0.2.91 · this file is generated from the same content as the page, so the two cannot drift. Every page on this site has a `.md` twin; internal links below point at them.*
...
The test for the word is what happens when you zoom. **If every level has the same node types, the same verbs and the same rules, you are in a hierarchy** — a folder tree is the clean example: folders inside folders inside folders, one schema all the way down. Depth gives you more detail but no new meaning, because there is only ever one verb, `contains`, and nothing at level ten can relate to anything at level three except by sitting inside it. **It becomes fractal at the moment zooming in lands you somewhere different** — a node whose inside has its own types, its own verbs, its own taxonomy, a new format or a special case — and that new world is still joined by an edge to the one you came from. Now **the deeper you go, the more you learn**: every altitude brings its own vocabulary of relationships, every node can connect to nodes in any other world, and the connectivity compounds instead of merely nesting. Graphs of graphs, ontologies of ontologies: the plural is the point. A document can contain a paragraph that is a whole mini-world of its own, with more definition than the document around it, because somebody needed that granularity there and nowhere else.

Put precisely, the cross-domain behaviour means one thing: **the core meaning of a node is supplied by the ontology at the altitude where it sits**, and that ontology is free to change from one altitude to the next. *Article 9* means one thing in a graph of instruments (a provision that binds), another inside the regulation (a container of paragraphs and points), another to the paragraph that cites it (a source of definitions) — the node is the same, the meaning is the edges around it at that level. Nature is built this way and nobody finds it strange: the universe is a graph of galaxies, a galaxy of stars, a star system of planets, a planet of ecosystems, an ecosystem of organisms, an organism of cells, a cell of molecules, a molecule of atoms, an atom of particles — and at every altitude the vocabulary changes completely, from cosmology to geology to biology to chemistry to physics, while each level stays connected to the one above and below. No single schema describes a galaxy and a cell. A single grammar — things, and the named relationships between them — describes both.
